-- Comprehensive fix for match_document_sections function
...
BEGIN
  ...
  WITH embedding_similarity AS (
    ...
    WHERE 
      ki.status = 'READY' AND  -- Fixed: should be 'READY' not 'COMPLETED'
...
GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float, int) TO anon;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float, int) TO service_role;

-- Also grant with default parameters
GRANT EXECUTE ON FUNCTION public.match_document_sections(text) TO anon;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text) TO service_role;

GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float) TO anon;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float) TO authenticated;
GRANT EXECUTE ON FUNCTION public.match_document_sections(text, float) TO service_role;

-- Add comment
COMMENT ON FUNCTION public.match_document_sections IS 'Performs cosine similarity search on knowledge chunks using JSON arrays. SECURITY DEFINER function with proper permissions.';
